...
from cogs.mongo.db import users, shikigami, bounty, friendship
from cogs.startup import emoji_m, emoji_j, emoji_c, emoji_f, emoji_a, pluralize, emoji_sp, emoji_ssr, emoji_sr, emoji_r
...
async def evolve_shikigami(ctx, rarity, evo, user, query, count):
    ...
    if rarity == "SP":
        ...
        if count >= rarity_count:
            ...
            noun = pluralize('dupe', required)
            embed = discord.Embed(
                colour=discord.Colour(0xffe6a7),
                title="Insufficient shikigamis",
                description=f"{user.mention}, you lack {required} more {query} {noun} to evolve yours"
            )
            await ctx.channel.send(embed=embed)
