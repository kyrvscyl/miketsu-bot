...
class Encounter(commands.Cog):
    ...
    async def encounter(self, ctx):
        ...
        if daily.find_one({"key": "daily"},
                          {"_id": 0, f"{user.id}.encounter_pass": 1})[str(user.id)]["encounter_pass"] > 0:
            daily.update_one({"key": "daily"}, {"$inc": {f"{user.id}.encounter_pass": -1}})
    ...
    async def boss_roll(self, user, ctx):
        ...
        boss_alive = []
        for boss_name in boss.find({"$or": [{"discoverer": {"$eq": 0}}, {"current_hp": {"$gt": 0}}]},
                                   {"_id": 0, "boss": 1}):
        ...
        boss_profile = boss.find_one({"boss": boss_select},
                                     {"_id": 0, "challengers": 1, "level": 1, "total_hp": 1, "current_hp": 1,
                                      "damage_cap": 1, "boss_url": 1})
        ...
        embed.set_footer(text=f"Discovered by {user.name}", icon_url=user.avatar_url)
            ...
            try:
                ...
                if not str(user.id) in assembly_players:
                    ...
                    if boss.find_one({"boss": boss_select, "challengers.user_id": str(user.id)}, {"_id": 1}) is None:
                        boss.update_one({"boss": boss_select},
                                        {"$push": {"challengers": {"user_id": str(user.id), "damage": 0}}})
                        boss.update_one({"boss": boss_select}, {
                            "$inc": {"rewards.medals": 15, "rewards.jades": 50, "rewards.experience": 50,
                                     "rewards.coins": 50000}})
        ...
        if len(assembly_players) == 0:
            await asyncio.sleep(3)
            await ctx.channel.send(f":x: No players joined the assembly! Rare Boss {boss_select} fled.")
            self.client.get_command("encounter").reset_cooldown(ctx)
        else:
            await asyncio.sleep(3)
            await ctx.channel.send(f":video_game: Battle with {boss_select} starts!")
            async with ctx.channel.typing():
                await asyncio.sleep(3)
                await self.boss_assembly(boss_select, assembly_players, boss_damagecap, boss_basedmg,
                                         boss_url, ctx)
    ...
    async def boss_assembly(self, boss_select, assembly_players, boss_damagecap, boss_basedmg, boss_url, ctx):
        ...
        for player in assembly_players:
            ...
            boss.update_one({"boss": boss_select, "challengers.user_id": player}, {
                "$inc": {"challengers.$.damage": round(player_dmg, 0), "current_hp": -round(player_dmg, 0)}})

            msg = f"{self.client.get_user(int(player)).mention} {random.choice(attack_verb)} {boss_select}, " \
                f"dealing {round(player_dmg)} damage!"
            await ctx.channel.send(msg)
            await asyncio.sleep(3)

        # Boss HP no less than 0
        boss_profile_new = boss.find_one({"boss": boss_select},
                                         {"_id": 0, "current_hp": 1, "rewards": 1, "discoverer": 1})
    ...
    async def boss_check(self, assembly_players, boss_select, boss_url, boss_profile_new, ctx):
        ...
        if boss_currenthp > 0:
            ...
            description1 = f":dash: Rare Boss {boss_select} has fled with {round(boss_currenthp)} remaining Hp"

            description2 = f":money_with_wings: Stealing {boss_jadesteal:,d}{emoji_j} & " \
            ...
            boss.update_one({"boss": boss_select}, {
                "$inc": {"rewards.jades": boss_jadesteal, "rewards.coins": boss_coinsteal}})

            await ctx.channel.send(embed=embed)
            self.client.get_command("encounter").reset_cooldown(ctx)

        else:
            for damage in boss.aggregate([{"$match": {"boss": boss_select}}, {"$unwind": {"path": "$challengers"}},
                                          {"$group": {"_id": "", "total_damage": {"$sum": "$challengers.damage"}}},
                                          {"$project": {"_id": 0}}]):
            ...
            for data in boss.aggregate([{"$match": {"boss": boss_select}}, {"$unwind": {"path": "$challengers"}},
                                        {"$project": {"_id": 0, "challengers": 1}}]):
    ...
    async def boss_defeat(self, boss_select, rewards_zip, boss_url, boss_profile_new, ctx):
        ...
        embed = discord.Embed(color=0xffff80, title=":confetti_ball: Boss Defeat Rewards!")
        embed.set_thumbnail(url=boss_url)
        msg = f"{self.client.get_user(int(discoverer)).mention} earned an extra 100{emoji_j}, 50,000{emoji_c}, " \
    ...
    async def boss_info(self, ctx, *args):
        ...
        try:
            boss_profile = boss.find_one({"boss": query},
                                         {"_id": 0, "level": 1, "total_hp": 1, "current_hp": 1, "rewards": 1,
                                          "discoverer": 1})
